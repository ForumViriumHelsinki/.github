...
globs: ["**/deploy/values.yaml", "**/deploy/**", "**/*feature-flag*/**", "**/*flags*.{ts,py}", "**/.github/workflows/**"]
---
...
Setting `featureFlags.enabled: true` in `helm-webapp` values automatically injects `FEATURE_FLAG_ENDPOINT` (pointing to the internal relay proxy), plus `FEATURE_FLAG_TIMEOUT` and `FEATURE_FLAG_REFRESH_INTERVAL`.
...
externalSecret:
  ...
  data:
    - secretKey: GOFF_CLIENT_ID
      remoteRef:
        key: my_app_goff_client_id  # <app>_goff_client_id (hyphens → underscores), provisioned in infrastructure gcp/*_secrets.tf
```
...
api.set_provider(GoFeatureFlagProvider(
    options=GoFeatureFlagOptions(
        endpoint=os.environ.get("FEATURE_FLAG_ENDPOINT"),
