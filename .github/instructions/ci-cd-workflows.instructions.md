...
Outputs:
- `release_created` — `true` when this run created a release for the root component, `false` otherwise. Path components of a monorepo config are not forwarded.
- `tag_name` — tag of the release this run created; empty when none was created.
- `missed_release` — `true` when the pushed range held releasable commits but release-please produced neither a release PR nor a release; `false` when it held none. Empty when the guard did not run: it runs only on `push` events, with `missed-release-check` not `off`, and only when no release or release PR was created.
