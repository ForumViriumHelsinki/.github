...
#
# With no argument it runs the fixtures against EVERY workflow that runs
# anthropics/claude-code-action with --json-schema, so a new analysis
...
if [ $# -eq 0 ]; then
  ...
  for f in .github/workflows/*.yml workflow-templates/*.yml; do
    ...
    if out="$(bash "$0" "$f" 2>&1)"; then
      n="$(printf '%s\n' "$out" | sed -n 's/^publish-findings: \([0-9][0-9]*\) assertion(s) passed\.$/\1/p')"
      total=$((total + ${n:-0}))
    else
      printf '%s\n' "$out"
      status=1
  ...
  if [ "$targets" -eq 0 ]; then
    echo "FATAL: no workflow runs claude-code-action with --json-schema; nothing to check" >&2
    exit 1
  fi
  if [ "$status" -ne 0 ]; then
    printf 'FAIL: publish-findings (%d workflow(s) checked)\n' "$targets"
    exit 1
  fi
  printf 'publish-findings: %d workflow(s) checked\n' "$targets"
  printf 'PASS: publish-findings (%d assertion(s))\n' "$total"
  exit 0
fi
