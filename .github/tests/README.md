...
- **Path and shape.** `.github/tests/<name>/run.sh`, `#!/usr/bin/env bash`, `set -euo pipefail`, `cd "$(git rev-parse --show-toplevel)"`, executable, no arguments required. Exit non-zero on any failure and print `PASS: <name> (<n> assertion(s))` on success.
...
- **Scan the class, not the file.** Loop over every `reusable-*.yml` (and `workflow-templates/*.yml` where relevant) so a future workflow that reintroduces the defect is caught. Assert each scan found at least one item before checking items.
