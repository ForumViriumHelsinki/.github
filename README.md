...
`.rulesync/.aiignore` lists `.rulesync/` and `rulesync.jsonc`, so the generated `.claude/settings.json` denies `Read` on both. In a Claude Code session the rule sources and the config read as permission errors rather than as missing files; inspect them through `git show HEAD:.rulesync/rules/<file>.md` instead. Generation writes that `deny` list into an existing `.claude/settings.json` and leaves the rest of the file — `allow`, `enabledPlugins`, everything else — untouched. It replaces the `deny` list rather than merging into it: a consuming repo's own entries, such as `Read(.env)`, are dropped, so read the sync PR's `.claude/settings.json` diff before merging.
