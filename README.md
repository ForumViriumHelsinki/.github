...
| `rulesync.jsonc` | rulesync targets and features; a consuming repo needs its own copy |
...
A consuming repo needs two files. Both are committed to the consuming repo, not to this one.

1. **`rulesync.jsonc`** — copy this repository's file verbatim. It selects the targets and features to generate:
...
2. **`.github/workflows/sync-ai-rules.yml`** — a caller for `reusable-sync-ai-rules.yml`. The reusable workflow declares only `workflow_call`, so the caller owns the trigger. The starter workflow in `workflow-templates/sync-ai-rules.yml` (also offered in the repo's Actions → New workflow page) runs weekly and on demand:
...
**`rulesync.jsonc` is not optional, and its absence fails quietly.** The workflow runs `npx rulesync@latest generate` with no `--targets` flag, so with no config file to read the run resolves to the `agentsmd` target alone — which no rule's frontmatter lists. Run against this repository's rules in an otherwise empty directory it reports `All files are up to date (rules)` and writes zero files, at exit 0. The sync PR still opens, because the fetched `.rulesync/` directory counts as a change; it just contains no `.claude/rules/`, `.github/instructions/`, `.cursor/rules/` or `.agents/rules/` files. If a sync PR touches only `.rulesync/`, the missing config is why.
